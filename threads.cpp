...
int main(){
  ...
  for ( i = 0; i<n ; i++)
  {
        ...
        if ( np == 0 && nc == n-1)probability = 0;

        // cout<<"4.2"<<endl;

        if ( nc == 0 && np == n-1 )probability = 1;
        ...
        switch(probability){
          ...
          case 1 :
            ...
            if(pthread_create( &(node.id), NULL, croutine, &(arr[i]) ) != 0 ){
              perror("THREAD CREATION ERROR : ");
            }
            cout<<"Consumer"<<endl;
            nc++;
 ...
 cout<<"In main(): joined scheduler"<<endl;
  ...
  cout<<"Total Jobs Executed: "<<tj<<endl;
...
void* croutine(void* args){
  ...
  while(flag){
    ...
    int fg=0;
    // for(int i =0;i<n;i++){
    //   if(workers[i].type == 1);     // skip if its a consumer
